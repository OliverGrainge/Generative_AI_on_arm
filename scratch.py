            ...
            if isinstance(mod, torch.nn.Linear):
                ...
                if self.scheme is None and not _check_linear_int4_k(
                    ...
                    if self.padding_allowed:
                        ...
                        print(weight.shape, padded_in_features, in_features)
                        raise Exception
                    else:
...
if __name__ == "__main__":
    ...
    weights = torch.randn(out_features, in_features, dtype=precision)
    inputs = torch.randn(batch_size, in_features, dtype=precision)

    layer = nn.Linear(in_features, out_features, bias=False)
    qlayer = nn.Linear(in_features, out_features, bias=False)
    qlayer.weight.data = weights 
    layer.weight.data = weights

    model = nn.Sequential(layer)
    qmodel = nn.Sequential(qlayer)
    ...
    qmodel = quantizer.quantize(qmodel)
    qout = qmodel(inputs)
    out = model(inputs)
    print(qout.shape, out.shape)
    print(qout.flatten()[:5]) 
    print(out.flatten()[:5])
